					   ...
					   u32 weight, u32 queue)
{
	...
	void __iomem *ioaddr = hw->pcsr;
	u32 value = readl(ioaddr + MTL_TXQX_WEIGHT_BASE_ADDR(dwmac4_addrs, queue));
			      ...
			      u32 high_credit, u32 low_credit, u32 queue)
{
	...
	value = readl(ioaddr + MTL_SEND_SLP_CREDX_BASE_ADDR(dwmac4_addrs, queue));
	value &= ~MTL_SEND_SLP_CRED_SSC_MASK;
	value |= send_slope & MTL_SEND_SLP_CRED_SSC_MASK;
	writel(value, ioaddr + MTL_SEND_SLP_CREDX_BASE_ADDR(dwmac4_addrs, queue));
...
static int dwmac4_irq_mtl_status(struct stmmac_priv *priv, struct mac_device_info *hw, u32 chan)
{
	...
	if (mtl_int_qx_status & MTL_INT_QX(chan)) {
		...
		u32 status = readl(ioaddr + MTL_CHAN_INT_CTRL(dwmac4_addrs, chan));

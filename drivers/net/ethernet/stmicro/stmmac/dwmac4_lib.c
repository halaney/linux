...
{
	u32 value = readl(ioaddr + EMAC3_DMA_CHAN_INTR_ENA(chan));
	...
	writel(value, ioaddr + EMAC3_DMA_CHAN_INTR_ENA(chan));
}
...
void emac3__disable_dma_irq(void __iomem *ioaddr, u32 chan, bool rx, bool tx)
{
	u32 value = readl(ioaddr + EMAC3_DMA_CHAN_INTR_ENA(chan));
...
{
	u32 intr_status = readl(ioaddr + EMAC3_DMA_CHAN_STATUS(chan));
	u32 intr_en = readl(ioaddr + EMAC3_DMA_CHAN_INTR_ENA(chan));
	int ret = 0;
	...
	writel(intr_status & intr_en, ioaddr + EMAC3_DMA_CHAN_STATUS(chan));
	return ret;
}

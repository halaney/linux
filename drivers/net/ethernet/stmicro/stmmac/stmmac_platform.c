 ...
 * plat->mdio_bus_data is allocated. If there's no MDIO bus described and
 * it's not a fixed-link configuration assume that MDIO bus exists and
 * indicate it needs to be scanned by just allocating
 * plat->mdio_bus_data.
 *
			     ...
			     struct device_node *np, struct device *dev)
{
	 ...
	 * the bus to be scanned for devices if !fixed-link.
	 */
	if (plat->mdio_node || !of_phy_is_fixed_link(np)) {
